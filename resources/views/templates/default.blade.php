...
<head>
    ...
    <link href="https://fonts.googleapis.com/css2?family=Bebas+Neue&family=Concert+One&family=Odibee+Sans&display=swap"
        rel="stylesheet">
...
<body>
        ...
        <div class="row">
            ...
            <ul style="display: inline" class="nav navbar-nav">
                ...
                <li><a href="{{route('auth.signout')}}">
                        <button style="border-radius: 25px; margin-top: 12px;" class="btn"><i
        ...
        <footer class="fixed-bottom">
            <ul>
                ...
                <li>
                    ...
                    <a href="{{route('friends.Solicitacoes')}}"><i class="fas fa-users"></i></a>
                    @else
                    <span>{{$friendRequests->count()}}</span><a href="{{route('friends.Solicitacoes')}}"><i
                ...
                <li>
                    <a href="{{route('home')}}">
                        <button
                            style="border-radius: 25px; background-color: var(--primary-color); color: var(--primaryText-color)"
                            class="btn"><i class="fa fa-home fa-lg"></i></button>
                    </a>
                </li>
                <li>
                    <a href="#">
    ...
    <button onclick="topFunction()" id="topo" title="Voltar ao topo"><i class="fa fa-arrow-up"
...
$(document).ready(function () {    
    $('.like').on('click', function(event) {
        ...
        $(`.likeCount-${postId}`).text(`Atualizando...`);
        $.ajax({
            type: "HEAD",
            url: '/status/' + postId + '/like',
            data: {
                "statusId": postId,
                _token: "{{ csrf_token() }}",
            }
        ...
        $.ajax({
            type: "GET",
            url: '/status/' + postId + '/likes',
            data: {
                "statusId": postId,
            },
            success: function(response){
                $(`.likeCount-${postId}`).text(`${response}`);
            }
        });
    })
});
